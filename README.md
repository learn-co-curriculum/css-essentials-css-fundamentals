...
You know the basics of CSS and how to use it to style HTML pages. But you want
to customize your presentation further. Fortunately, CSS has several fundamental
powers to explore, including selector types, color values and comments.
...
want to style. Inside the curly braces we declare the property we want to change
and, after the colon, we set the value we want to change that property to. We
end the declaration with a semicolon. Here, we are selecting the `p` element and
displaying its color as blue.

## Identify CSS Use Formats

There are three distinct formats that you might see CSS appear in: inline,
...
In this case, only `p` elements within the `header` element will recieve the
styling.
...
The general sibling selector (sometimes called the preceded selector) will style all matched elements preceding after the selector name. 
...
Here we want to find `input` elements, but only those with a `type` value that matches "text." For those elements that fit the requirements, the browswer will then apply the width we want. There are many different ways to use this type of selector with various combinations of operations and attribute values, so you can refer to the resources to explore them all.
...
Most often developers use a set of numbers called hexadecimals, which represents a wide range of colors. Hex colors begin with `#` and are followed by, generally, 6 numbers, but some of these numbers are actually letters. The lowest single digit number in hex is 0 and the highest single digit number is f. This table might help to visualize what we mean by this.
...
Here we've set our `p` elements to the color white, the maximum of all RGB values (255).
...
Everything in between the `/* */` is a CSS comment. The browser will not pay attention to these comments, but they can be useful for us to add explanations or reminders along side our CSS code.
